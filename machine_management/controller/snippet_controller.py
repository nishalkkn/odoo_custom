...
class DynamicSnippets(http.Controller):
    ...
    def top_selling(self):
        ...
        # machine_id=request.env['machine.management'].sudo().search([])
        # machine=[]
        # for rec in machine_id:
        #
        ...
        public_categ_id = request.env['product.public.category'].sudo().search_read([], ['name', 'website_id'])
        products = []
        public_categories = []
        for category in public_categ_id:
            products_search_read = request.env['product.template'].with_user(1).search_read(
                [('is_published', '=', True),
